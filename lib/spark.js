...
(function( window, $, undefined ){
    ...
    var View = (function(){
        ...
        function _setTemplate( template ){
            ...
            template = template.replace( VAR_REGEX, _variableObserver.bind( self ) );
            return self;
        }

        function _variableObserver( match, variable, flags, offset, template ){
            if( !FLAG_IGNORE.test( flags ) ){
                _observe.call( this, variable.split( '.' ) );
            }
        ...
        function _observe( path ){
            ...
            var obj         = self[ VIEW_LOCALS ][ path[ 0 ] ];
            for( var i = 1; obj && i < lastIndex; ++i ){
                obj = obj( path[ i ] );
            }
            if( !obj ){
